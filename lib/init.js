...
ep.addExtract(/s\d{1,2}e(\d{2,3})/i, 90);       // "s01e01"
ep.addExtract(/s(?:19|20)\d\de(\d{2,3})/i, 90); // "s1980e01"
ep.addExtract(/s\d{1,2}xe(\d{2,3})/i, 90);      // "S01xE01"
ep.addExtract(/s(?:19|20)\d\dxe(\d{2,3})/i, 90);// "S1980xE01"
ep.addExtract(/\bseason\W+\d{1,2}\W+episode\W+(\d{1,3})\W/i, 80); // "season 01 episode 01"
ep.addExtract(/\bseason\W+(?:19|20)\d\d\W+episode\W+(\d{1,3})\W/i, 80); // "season 1980 episode 01"
ep.addExtract(/\bepisode\W+(\d{2,3})\W/i, 70);  // "episode 01" or "episode 001"
ep.addExtract(/s\d{1,2}\We(\d{2,3})/i, 80);     // "s01 e01"
ep.addExtract(/s(?:19|20)\d\d\We(\d{2,3})/i, 80); // "s1980 e01"
ep.addExtract(/\b(\d+)\Wof\W\d+\b/i, 60);       // " 1 of 14 "
ep.addExtract(/e(\d\d\d)/i, 60);                // "e001"
ep.addExtract(/\bE(\d{2,3})\b/, 60);               // " E01 "
ep.addExtract(/\b\dx(\d{2,3})\b/, 50);             // " 1x01 "
ep.addExtract(/\b\d\dx(\d{2,3})\b/, 50);           // " 01x01 "
ep.addExtract(/\b\dx(\d)\b/, 30);               // " 1x1 "

ep.addExtract(/\dx(\d{2,3})\b/, 5);               // "1x01 "
...
season.addExtract(/\b(19\d\d)\W\d+\Wof\W\d+\b/i, 80); // " 1997 14 of 21 "
season.addExtract(/\b(20\d\d)\W\d+\Wof\W\d+\b/i, 80); // " 2012 14 of 21 "
season.addExtract(/s(\d{1,2})e\d{2,3}/i, 90);       // "s01e01"
season.addExtract(/s((?:19|20)\d\d)e\d{2,3}/i, 90); // "s1980e01"
season.addExtract(/s(\d{1,2})xe\d{2,3}/i, 90);      // "S01xE01"
season.addExtract(/s((?:19|20)\d\d)xe\d{2,3}/i, 90);// "S1980xE01"
season.addExtract(/\bseason\W+(\d{1,2})\W+episode\W+\d{1,3}\b/i, 80); // "season 01 episode 01"
season.addExtract(/\bseason\W+((?:19|20)\d\d)\W+episode\W+\d{1,3}\b/i, 80); // "season 1980 episode 01"
season.addExtract(/s(\d{1,2})\We\d{2,3}/i, 80);      // "s01 e01"
season.addExtract(/s((?:19|20)\d\d)\We\d{2,3}/i, 80);// "s1980 e01"
season.addExtract(/\bS(\d{2,3})\b/, 60);               // " S01 "
season.addExtract(/\bS((?:19|20)\d\d)\b/, 60);      // " S1980 "
season.addExtract(/\b(\d)x\d{2,3}\b/, 50);             // " 1x01 "
season.addExtract(/\b((?:19|20)\d\d)x\d{2,3}\b/, 50);  // " 1980x01 "
season.addExtract(/\b(\d\d)x\d{2,3}\b/, 50);           // " 01x01 "
season.addExtract(/\b((?:19|20)\d\d)x\d{2,3}\b/, 50);  // " 1980x01 "
season.addExtract(/\b(\d)x\d\b/, 30);               // " 1x1 "
season.addExtract(/\b((?:19|20)\d\d)x\d\b/, 30);    // " 1980x1 "

season.addExtract(/(\d\d)x\d{2,3}\b/, 6);           // "01x01 "
season.addExtract(/((?:19|20)\d\d)x\d{2,3}\b/, 6);  // "1980x01 "
season.addExtract(/(\d)x\d\b/, 5);               // "1x1 "
season.addExtract(/((?:19|20)\d\d)x\d\b/, 5);    // "1980x1 "
...
m = year.addExtract(/\b((?:19|20)\d{2})\b/, 60); // " 2012 "
...
for (const [ id, name ] of require('./channels.json')) {
  channel.addMatch(name, id + '.WEBRip', 50);
  channel.addMatch(name, id + '.WEB-DL', 50);
}
...
res.addMatch('2160i', /(?:\d{3,}(?:\\|\/|x|\*))?2160i/i, 60).setQuality(400);
res.addMatch('2160p', /(?:\d{3,}(?:\\|\/|x|\*))?(?:2160p?x?|\b4k\b)/i, 60).setQuality(400);
...
codec.addMatch('h264', /\b[hx][-\. ]?264(?:-AVC)?/i, 60).setQuality(100);
codec.addMatch('h265', /\b[hx][-\. ]?265(?:-HEVC)?/i, 60).setQuality(150);
...
profile.addMatch('Dolby Vision', /\bDV\b/, 25).setQuality(15).require('video_codec');
profile.addMatch('8bit', /8.?bit/, 10).setQuality(15).positionAfter('title');
profile.addMatch('HDR', /\bHDR\b/, 12).setQuality(15).require('video_codec');
...
audio.addMatch('Dolby Atmos', /\bAtmos\b/, 30).setQuality(55);
...
subtitle.addMatch('scandinavian', 'NORDiC', 90).setQuality(-25);
...
language.addMatch('ger', 'GERMAN', 80);
...
website.addExtract(/(?=[A-Z0-9]+\.[A-Z0-9]{2,})((?:[A-Z0-9]{2,})\.org)\b/gi, 10);        // "bla.bla.org"
website.addExtract(/(?=[A-Z0-9]+\.[A-Z0-9]{2,})((?:[A-Z0-9]{2,})\.com)\b/gi, 10);        // "bla.bla.com"
website.addExtract(/(?=[A-Z0-9]+\.[A-Z0-9]{2,})((?:[A-Z0-9]{2,})\.ru)\b/gi, 10);         // "bla.bla.ru"
website.addExtract(/(?=[A-Z0-9]+\.[A-Z0-9]{2,})((?:[A-Z0-9]{2,})\.tv)\b/gi, 10);         // "bla.bla.tv"
